...
--
-- Differences from the MEGA65 framework.vhd this is derived from:
--  * MEGA65 smart keyboard (kio serial protocol) replaced by a scanner for a
--    real C64 keyboard on the Wukong's J12 header (m2m_keyb_wukong)
--  * HyperRAM controller replaced by the Wukong's DDR3 through UberDDR3
--    (ddr3_wrapper_wukong); the Avalon-MM fabric side is unchanged
--  * No I2C peripherals on the board: rtc_wrapper kept for firmware
--    compatibility, buses tied to idle
--  * VGA/VDAC outputs kept in the entity but unconnected at the board top
--
...
port (
   ...
   -- Built-in DDR3 (through UberDDR3), replacing the MEGA65's HyperRAM
   ...
   ddr3_calib_complete_o   : out   std_logic;
...
end entity framework_wukong;

architecture synthesis of framework_wukong is
...
begin
   ...
   ---------------------------------------------------------------------------------------------------------------
   -- DDR3 controller (UberDDR3), presenting the HyperRAM controller's Avalon interface
   ---------------------------------------------------------------------------------------------------------------
   ...
   i_ddr3_wrapper : entity work.ddr3_wrapper_wukong
      port map (
         ...
         calib_complete_o    => ddr3_calib_complete_o,
